'use client';
import { CheckCircle2, Clock, Layers, BarChart2, AlertTriangle } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const sprintIssues = activeSprint
    ? Object.values(issues).filter((i) => i.sprintId === activeSprint.id && i.type !== 'subtask')
    : [];

  const backlogCount = projectIssues.filter((i) => !i.sprintId).length;
  const openCount = projectIssues.filter((i) => ['todo', 'in_progress', 'review'].includes(i.status)).length;
  ...
  const urgentCount = projectIssues.filter((i) => i.priority === 'urgent' && i.status !== 'done').length;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <Header
        title={project ? `${project.emoji} ${project.name}` : 'Dashboard'}
      ...
      <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <StatsCard
            ...
            subtitle={activeSprint?.name ?? 'No active sprint'}
            icon={<Clock size={20} />}
            iconBg="#FFFBEB"
            iconColor="#D97706"
          />
          <StatsCard
            ...
            trend={
              sprintIssues.length > 0
                ? { value: Math.round((doneCount / sprintIssues.length) * 100), label: '% complete' }
                : undefined
            }
          />
          <StatsCard
        ...
        {urgentCount > 0 && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-6">
            ...
            <p className="text-sm text-red-700">
              <span className="font-semibold">{urgentCount} urgent issue{urgentCount > 1 ? 's' : ''}</span>{' '}
        ...
        <div className="mt-4 bg-white rounded-xl border border-gray-100 shadow-card p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Epics', value: projectIssues.filter((i) => i.type === 'epic').length, color: '#7C3AED' },
              { label: 'Stories', value: projectIssues.filter((i) => i.type === 'story').length, color: '#2563EB' },
              { label: 'Bugs', value: projectIssues.filter((i) => i.type === 'bug').length, color: '#DC2626' },
            ].map((item) => (
