...
export function SprintHeader() {
  const { getAllSprints, getSprintById } = useSprintStore();
  const { activeSprintId, setActiveSprint } = useUIStore();
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={sprint.status === 'active' ? 'info' : sprint.status === 'completed' ? 'success' : 'default'}
              >
      ...
      {sprint.status === 'active' && (
        <div className="mt-3">
          ...
          <p className="text-[10px] text-gray-400 mt-1">
            {sp.completed}/{sp.total} story points completed
          </p>
        </div>
      )}
    </div>
